...
void make_button(button *button_info)
{
    ...
    else if(button_info->mnemonic->Idelem!=-1) btn=gtk_button_new_with_mnemonic(button_info->mnemonic->info);
    else if(button_info->button_from_icon_name->Idelem !=-1) btn=gtk_button_new_from_icon_name(button_info->button_from_icon_name->info,button_info->icon_siz);
...
void make_image(image *image_info)
{
    ...
    if(image_info->file_name->Idelem!=-1) im=gtk_image_new_from_file(image_info->file_name->info);
    ...
    gtk_image_set_pixel_size(GTK_IMAGE(im),image_info->pixel_size);
    image_info->image=im;
}
...
menu_item *remplir_menu_item(FILE *f1)
{
    ...
    if(strcmp(liste->attribut->info,"label")==0)remplir_info(in->label,liste->valeur->info);
    else remplir_info(in->mnemonic,liste->valeur->info);
    p=in;
    do {
        fscanf(f1,"<%30[^ ] ",mot);
        if(strcmp(mot,"item")==0)
        {
            ...
            if(strcmp(liste->attribut->info,"label")==0)remplir_info(ne->label,liste->valeur->info);
            else remplir_info(ne->mnemonic,liste->valeur->info);
            if(k==0)
            {
                p->but=ne;
                ne->top=p;
...
entry *crrer_initialser_entry()
{
    ...
    remplir_info(ent->text,"default");
    ent->placeholder=creer_initialiser_info();
    remplir_info(ent->placeholder,"default");
    ent->icon_name=creer_initialiser_info();
    remplir_info(ent->icon_name,"document-open");
    ent->tool_tip_text_icon=creer_initialiser_info();
    remplir_info(ent->tool_tip_text_icon,"default");
    ent->max_entry=80;
    ent->frame=TRUE;
    ent->alignement=0;
    ent->overwriten=TRUE;
...
void make_entry(entry *entry_info)
{
    ...
    gtk_entry_set_text(GTK_ENTRY(ent),entry_info->text->info);
    ...
    gtk_entry_set_placeholder_text(GTK_ENTRY(ent),entry_info->placeholder->info);
    ...
    gtk_entry_set_icon_tooltip_text(GTK_ENTRY(ent),entry_info->icon_position,entry_info->tool_tip_text_icon->info);
...
GtkWidget *principale_function(FILE *f1,tab_glist *info)
{
    ...
    }else if(strcmp(mot,"box")==0)
    {
        ...
        do {
            if(strcmp(mot1,"pack-start")==0){
                gtk_box_pack_start(GTK_BOX(sbx->box1),principale_function(f1,info),FALSE,FALSE,0);
            }
            else if(strcmp(mot1,"pack-end")==0)
            {
                gtk_box_pack_end(GTK_BOX(sbx->box1),principale_function(f1,info),FALSE,FALSE,0);
            }
            fscanf(f1,"<%35[^>]>\n",mot2);
            epurer_blan(f1);
            fscanf(f1,"<%35[^ ] ",mot);
    ...
    }else if(strcmp(mot,"grid")==0)
    {
        ...
        epurer_blan(f1);
        do {
            gtk_grid_attach(GTK_GRID(sgr->gri), principale_function(f1,info),k,j,w,h);
            fscanf(f1,"<%35[^>]>\n",mot2);
            epurer_blan(f1);
            fscanf(f1,"<%35[^ ] ",mot);
            if(strcmp(mot,"child")==0)
    ...
    }else if(strcmp(mot,"mnbutton")==0)
    {
        ...
        gtk_menu_button_set_popup(GTK_MENU_BUTTON(smnbt->menu_butt), principale_function(f1,info));
        fscanf(f1,"<%35[^>]>\n",mot1);
        epurer_blan(f1);
...
int main(int argc,char *argv[])
{
    ...
    FILE *f1=NULL;
    f1= fopen("D:\\C projects\\GTK\\projet_xml\\text1.txt","r");
    if(!f1)
    {
